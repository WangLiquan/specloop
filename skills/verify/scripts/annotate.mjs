...
function renderSpecHtml(spec2) {
  ...
  const body = [
    ...
    `<p class="kicker">\u89C4\u683C\u8BF4\u660E \xB7 SPEC</p>`,
...
function verdictCounts(criteria, verdicts2) {
...
function renderAwareness(awareness) {
  ...
  const items = awareness.map((a) => {
    ...
    return `<div class="aware ${sev}" id="${escHtml(a.id)}"><span class="aw-ic">${ic}</span><p class="aw-t">${enrich(a.text)}</p></div>`;
  }).join("");
  return `<section class="block awareness" id="awareness"><h2><span class="card-n">\u25B2</span>\u4F60\u9700\u8981\u77E5\u9053</h2>${items}</section>`;
}
function renderInner(s) {
  ...
  if (s.type === "table") {
    return `<table>${s.rows.map(
      (r, i) => `<tr${i === 0 ? ' class="thead"' : ""}>${r.map((c) => `<td>${enrich(c)}</td>`).join("")}</tr>`
    ).join("")}</table>`;
  }
  return "";
}
...
function renderFoldZone(summary, foldSections, criteria, decidedDecisions = [], verdicts2 = [], assumptions = []) {
  ...
  if (criteria.length) {
    const verified = verdicts2.length > 0;
    const label = `\u9A8C\u6536\u70B9 <span class="fold-n">${criteria.length}</span>${verified ? ' <span class="fold-badge">\u5DF2\u9A8C</span>' : ""}`;
    folds.push(detail("criteria", label, criteriaInner(criteria, verdicts2), "criteria", verified));
  }
...
function criteriaInner(criteria, verdicts2 = []) {
  ...
  const byId = new Map(verdicts2.map((v) => [v.criterionId, v]));
  const verified = verdicts2.length > 0;
  const cov = verified ? coverageBar(criteria, verdicts2) : "";
...
function coverageBar(criteria, verdicts2) {
  const counts = verdictCounts(criteria, verdicts2);
...
function renderNav(spec2, openSections, foldSections, openDecisions, awareness, criteria, decidedDecisions = []) {
  ...
  return `<nav class="toc"><div class="brand"><span class="mark">\u25C6</span> SpecForge</div><p class="toc-title">\u76EE\u5F55</p>${links.join("")}</nav>`;
}
...
// lib/cli/annotate-main.mjs
var [, , specHtmlPath, verdictsPath] = process.argv;
...
var merged = { ...spec, verdicts, verifiedAt: (/* @__PURE__ */ new Date()).toISOString() };
