  ...
  properties: {
    ...
    decisions: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "question", "resolution"],
        properties: {
          id: { type: "string", pattern: "^D-[0-9]+$" },
          question: { type: "string", minLength: 1 },
          options: { type: "array", items: { type: "string" } },
          resolution: { type: "string", minLength: 1 },
          status: { enum: ["decided", "open"] },
          rationale: { type: "string" }
        }
      }
    },
...
function validateSpec(obj) {
  ...
  if (!validateSchema(obj)) {
    for (const e of validateSchema.errors) errors2.push(`${e.instancePath || "/"} ${e.message}`);
...
// lib/escape.mjs
function htmlSafeJsonIsland(obj) {
...
// lib/theme.mjs
var CSP_META = `<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src 'self' data:; base-uri 'none'; form-action 'none'">`;
var THEME_CSS = `
:root{
...
function renderReportHtml(spec2, verdicts2, specHash2) {
  const byId = new Map(verdicts2.map((v) => [v.criterionId, v]));
  const counts = { pass: 0, partial: 0, fail: 0, na: 0 };
  for (const c of spec2.criteria) {
    const v = byId.get(c.id);
    counts[v ? v.status : "na"]++;
  }
  const total = spec2.criteria.length || 1;
  ...
  const rows = spec2.criteria.map((c) => {
    ...
    const ev = v && v.evidence.length ? v.evidence.map((e) => `<div class="ev"><span><b>${escHtml(e.file)}</b>:${escHtml(e.line)}${e.note ? " \u2014 " + escHtml(e.note) : ""}</span></div>`).join("") : `<div class="ev"><span>${escHtml(v && v.missingEvidenceReason || "\u65E0\u8BC1\u636E")}</span></div>`;
    return `<div class="verdict ${status}" id="${escHtml(c.id)}"><div class="v-h"><strong class="ac-id">${escHtml(c.id)}</strong><span class="vstat ${status}">${escHtml(status)}</span><span class="v-t">${escHtml(c.text)}</span></div><div class="v-meta"><span>mode=<b>${escHtml(v ? v.verificationMode : "na")}</b></span><span>confidence=<b>${escHtml(v ? v.confidence : "-")}</b></span></div>${ev}<p>${escHtml(v ? v.explanation : "\u672A\u7ED9\u51FA\u5224\u5B9A")}</p></div>`;
  }).join("\n");
  const body = `<header class="masthead">
<p class="kicker">\u6821\u9A8C\u62A5\u544A \xB7 REPORT</p>
<h1>${escHtml(spec2.meta.title)} \xB7 \u6821\u9A8C\u62A5\u544A</h1>
<p class="meta">bound spec hash <code>${escHtml(specHash2)}</code> \xB7 revision ${escHtml(spec2.meta.revision)}</p>
</header>
...
function specHash(spec2) {
  const contract = {
    schemaVersion: spec2.schemaVersion,
    summary: spec2.summary,
    sections: spec2.sections,
...
// lib/cli/render-report-main.mjs
var [, , specHtmlPath, verdictsPath, outPath] = process.argv;
...
var verdicts = JSON.parse(readFileSync(verdictsPath, "utf8"));
var { ok, errors } = validateSpec({ ...spec, verdicts });
if (!ok) {
