...
function renderReportHtml(spec2, verdicts2, specHash2) {
  ...
  const rows = spec2.criteria.map((c) => {
    ...
    const ev = v && v.evidence.length ? v.evidence.map((e) => `<div class="ev">${escHtml(e.file)}:${escHtml(e.line)}${e.note ? " \u2014 " + escHtml(e.note) : ""}</div>`).join("") : `<div class="ev">${escHtml(v && v.missingEvidenceReason || "\u65E0\u8BC1\u636E")}</div>`;
    return `<div class="verdict ${status}"><strong>${escHtml(c.id)}</strong> [${escHtml(status)}] ${escHtml(c.text)}
<div class="meta">mode=${escHtml(v ? v.verificationMode : "na")} \xB7 confidence=${escHtml(v ? v.confidence : "-")}</div>
${ev}
<p>${escHtml(v ? v.explanation : "\u672A\u7ED9\u51FA\u5224\u5B9A")}</p></div>`;
  }).join("\n");
  const body = `<h1>${escHtml(spec2.meta.title)} \xB7 \u6821\u9A8C\u62A5\u544A</h1>
<p class="meta">bound spec hash: ${escHtml(specHash2)} \xB7 revision ${escHtml(spec2.meta.revision)}</p>
