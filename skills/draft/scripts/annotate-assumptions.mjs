...
function renderSpecHtml(spec2) {
  ...
  const body = [
    ...
    renderFoldZone(spec2.summary, foldSections, criteria, decidedDecisions, spec2.verdicts || [], assumptions2)
  ].filter(Boolean).join("\n");
...
function assumptionItems(assumptions2) {
  return assumptions2.map((a) => {
    const ev = effectiveVerified(a);
...
function renderFoldZone(summary, foldSections, criteria, decidedDecisions = [], verdicts = [], assumptions2 = []) {
  const folds = [];
  if (summary) {
  ...
  if (assumptions2.length) {
    folds.push(detail(
      "assumptions",
      `\u73B0\u72B6\u5047\u8BBE <span class="fold-n">${assumptions2.length}</span>`,
      assumptionItems(assumptions2)
    ));
  }
