...
function renderSpecHtml(spec2) {
  const body = [
    ...
    `<p class="summary">${escHtml(spec2.summary)}</p>`,
    ...spec2.sections.map(renderSection),
    renderCriteria(spec2.criteria)
  ].join("\n");
  return page(spec2.meta.title, body, spec2);
}
...
function renderCriteria(criteria) {
  const items = criteria.map(
    (c) => `<li data-id="${escHtml(c.id)}"><strong>${escHtml(c.id)}</strong><span class="pri">${escHtml(c.priority)}</span>${escHtml(c.text)}</li>`
  ).join("");
