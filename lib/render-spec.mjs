...
export function renderSpecHtml(spec) {
  ...
  const body = [
    ...
    renderFoldZone(spec.summary, foldSections, criteria, decidedDecisions, spec.verdicts || [])
  ].filter(Boolean).join('\n');
...
// ── 现状假设：spec 对现有代码/系统的事实断言 + 核验态。置顶供人看核验结果。 ──
...
function renderAssumptions(assumptions) {
  ...
  return `<section class="block decisions" id="assumptions">` +
    `<h2><span class="card-n">⊙</span>现状假设（待核验）</h2>${items}</section>`;
}
...
function renderNav(spec, openSections, foldSections, openDecisions, awareness, criteria, decidedDecisions = []) {
  ...
  if ((spec.assumptions || []).length) links.push(navLink('#assumptions', '⊙', '现状假设', true));
