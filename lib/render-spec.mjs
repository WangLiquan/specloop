...
// 信息架构：spec.html 是给「人 review/拍板」看的——决策点 / 知情项 / 流程置顶展开，
// 概述 / 其余小节 / 验收点（机器契约，给 specforge-verify）默认折叠进 <details>（纯 HTML，无脚本）。
export function renderSpecHtml(spec) {
  ...
  const body = [
    ...
    renderFoldZone(spec.summary, foldSections, criteria)
  ].filter(Boolean).join('\n');
  return page(spec.meta.title, renderNav(spec, openSections, foldSections, decisions, awareness, criteria), body, spec);
}
...
function renderDecisions(decisions) {
  ...
  const items = decisions.map(d => {
    ...
    return `<div class="decision ${status}" id="${escHtml(d.id)}">` +
      ...
      `<p class="d-res"><span class="d-res-l">拍板</span>${enrich(d.resolution)}</p>${why}</div>`;
  }).join('');
...
// 折叠区：概述 + 折叠小节 + 验收点，各包一个 <details>，默认收起
...
function renderNav(spec, openSections, foldSections, decisions, awareness, criteria) {
  const links = [];
