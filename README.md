...
[![CI](https://github.com/WangLiquan/specloop/actions/workflows/ci.yml/badge.svg)](https://github.com/WangLiquan/specloop/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](./LICENSE)

更轻的「定义需求 → 验证实现」闭环，做成两个 skill：

- **`specloop-draft`** —— 把一个模糊 idea，经访谈 + 拷问，变成一份**单文件可视化的 `*.spec.html`**，让你用最小成本看懂「AI 要做什么」并拍板。
- **`specloop-verify`** —— 拿着这份 spec.html 逐条比对你的代码，产出一份**可视化的 `*.report.html`**，告诉你哪条验收点真的做到了、哪条没有。
...
模型越强，多 skill 编排越显臃肿；人越依赖模型写码，markdown spec 对**人**的阅读成本反而越高。SpecLoop 押注三件不可替代物：
...
npx skills add WangLiquan/specloop --skill specloop-draft   # 只装其一
npx skills add WangLiquan/specloop -g               # 装到全局(所有项目可用)
```
